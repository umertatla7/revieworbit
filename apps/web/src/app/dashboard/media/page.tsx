...
type Config = {
  text: string; color: string; font_size: number; min_font_size: number; font_family: string;
...
const defaults: Config = {
  text: "{{customer_first_name}}", color: "#17201b", font_size: 72, min_font_size: 16,
...
export default function MediaPage() {
  ...
  async function save(event: React.FormEvent<HTMLFormElement>) {
    ...
    const values: Record<string, string> = {
      name, text: config.text, color: config.color, font_size: String(config.font_size), min_font_size: String(config.min_font_size), font_family: config.font_family,
  ...
  return <div className="mx-auto max-w-7xl">
    ...
    <div className="mt-7 grid gap-6 xl:grid-cols-[minmax(0,1.2fr)_minmax(360px,.8fr)]">
      ...
      <form onSubmit={save} className="panel space-y-4"><div><p className="eyebrow">Template settings</p><h2 className="mt-1 text-xl font-semibold">{editing ? "Refine template" : "Add personalized media"}</h2></div><label className="label">Template name<input className="field" required value={name} onChange={(event) => setName(event.target.value)} placeholder="Salon welcome board" /></label>{!editing && <label className="label">Background image<input ref={fileRef} className="field" type="file" name="background" accept="image/png,image/jpeg,image/webp" required onChange={(event) => chooseFile(event.target.files?.[0])} /><span className="mt-2 block text-xs font-normal text-ink/45">Use a clear sign, card, board, or open area where the name should appear.</span></label>}<label className="label">Text on image<input className="field font-mono" value={config.text} onChange={(event) => change("text", event.target.value)} /><span className="mt-2 block text-xs font-normal text-ink/45">Use {"{{customer_first_name}}"} or {"{{customer_name}}"}.</span></label>
        ...
        <div className="grid grid-cols-2 gap-3"><label className="label">Alignment<select className="field" value={config.align} onChange={(event) => change("align", event.target.value as Config["align"])}><option value="left">Left</option><option value="center">Center</option><option value="right">Right</option></select></label><label className="label">Maximum lines<select className="field" value={config.max_lines} onChange={(event) => change("max_lines", Number(event.target.value))}><option value="1">1 line</option><option value="2">2 lines</option><option value="3">3 lines</option></select></label><label className="label">Maximum size<input className="field" type="number" min="12" max="160" value={config.font_size} onChange={(event) => change("font_size", Number(event.target.value))} /></label><label className="label">Minimum size<input className="field" type="number" min="10" max="80" value={config.min_font_size} onChange={(event) => change("min_font_size", Number(event.target.value))} /></label></div><label className="label">Text color<input className="field h-11" type="color" value={config.color} onChange={(event) => change("color", event.target.value)} /></label>{!editing && <label className="flex items-start gap-3 rounded-lg border border-ink/10 p-3 text-sm"><input className="mt-1 accent-[#174d3b]" type="checkbox" checked={config.placement_mode === "auto"} onChange={(event) => setConfig((current) => ({ ...current, placement_mode: event.target.checked ? "auto" : "manual" }))} /><span><strong className="block">Suggest the initial writing area</strong><span className="text-xs leading-5 text-ink/50">ReviewOrbit finds a low-detail area first; drag all four corners for the final fit.</span></span></label>}<label className="label">Placement note <span className="font-normal text-ink/40">optional</span><textarea className="field min-h-20" value={config.placement_description ?? ""} onChange={(event) => change("placement_description", event.target.value)} placeholder="Example: Fit the name inside the white board held by the person." /></label><button type="submit" disabled={saving} className="button-primary w-full">{saving ? "Saving…" : editing ? "Save changes" : "Create media template"}</button>
      </form>
    </div>
