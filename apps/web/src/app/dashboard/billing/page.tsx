...
export default function BillingPage() {
  ...
  return <div className="mx-auto max-w-7xl">
    ...
    {!data.stripe_ready&&<Notice text="Online billing is temporarily unavailable while Breviews finishes its Stripe setup. Your current plan information is still shown." tone="warning"/>}
    ...
    {tab==="plans"&&<div>
      ...
      {customizing&&<div className="fixed inset-0 z-50 overflow-y-auto bg-ink/60 p-3 sm:p-8" role="dialog" aria-modal="true" aria-label="Customize your Breviews plan"><div className="mx-auto max-w-5xl"><CustomPlanBuilder quote={customQuote} busy={busy} canManage={canManage} calculate={calculateCustomPlan} activate={activateCustomPlan} close={()=>{quoteSequence.current++;setCustomizing(false);setCustomQuote(null);}}/></div></div>}
    </div>}

    {tab==="payment"&&<section className="mt-6 overflow-hidden rounded-2xl border border-ink/8 bg-white">
      <header className="flex flex-wrap items-center justify-between gap-4 border-b border-ink/8 p-6"><div><h2 className="text-xl font-semibold">Payment methods</h2><p className="mt-1 text-sm text-ink/45">Card details are entered in Stripe&apos;s secure form without leaving Breviews.</p></div>{canManage&&data.stripe_ready&&<button className="button-primary" disabled={Boolean(busy)} onClick={openPaymentMethod}>{busy==="payment"?"Opening secure form…":primary?"Change payment method":"Add payment method"}</button>}</header>
