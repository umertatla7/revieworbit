...
type Billing = { stripe_ready:boolean; has_stripe_customer:boolean; can_manage_billing:boolean; current_plan_code:string; subscription:Subscription|null; plans:Plan[]; invoices:Invoice[]; payment_methods:PaymentMethod[]; stripe_error:string|null };
...
export default function BillingPage() {
  ...
  const canManage=data.can_manage_billing&&!support;
  ...
  return <div className="mx-auto max-w-7xl">
    ...
    {support&&<Notice text="Billing is view-only during an admin support session. Only the business owner can open Stripe or change payment details." tone="warning"/>}
    ...
    {tab==="plans"&&<div>
      ...
      <div className="mt-6 grid gap-5 lg:grid-cols-3">{data.plans.map(plan=><PlanCard key={plan.id} plan={plan} interval={interval} active={hasSubscription&&current?.plan?.code===plan.code} stripeReady={data.stripe_ready} canManage={canManage} busy={busy} choose={()=>redirect("/api/v1/billing/checkout",{plan_id:plan.id,interval},plan.id)}/>)}</div>
    </div>}

    {tab==="payment"&&<section className="mt-6 overflow-hidden rounded-2xl border border-ink/8 bg-white">
      <header className="flex flex-wrap items-center justify-between gap-4 border-b border-ink/8 p-6"><div><h2 className="text-xl font-semibold">Payment methods</h2><p className="mt-1 text-sm text-ink/45">Card details are stored and updated securely by Stripe.</p></div>{canManage&&data.has_stripe_customer&&<button className="button-primary" disabled={Boolean(busy)} onClick={()=>redirect("/api/v1/billing/portal",{flow:"payment_method"},"payment")}>{busy==="payment"?"Opening Stripe…":primary?"Change payment method":"Add payment method"}</button>}</header>
      {data.payment_methods.length?<div className="grid gap-4 p-6 md:grid-cols-2">{data.payment_methods.map(method=><div key={method.id} className="rounded-2xl border border-ink/10 p-5"><div className="flex items-start justify-between gap-4"><div><p className="text-xs font-semibold uppercase tracking-widest text-ink/40">{capitalize(method.brand)} card</p><strong className="mt-3 block text-xl">•••• •••• •••• {method.last4}</strong><p className="mt-2 text-sm text-ink/45">Expires {pad(method.exp_month)}/{method.exp_year}</p></div>{method.is_default&&<span className="pill">Default</span>}</div></div>)}</div>:<Empty title="No payment method on file" body={data.has_stripe_customer?"Use Add payment method to securely add a card in Stripe.":"Choose a plan first. Stripe Checkout will securely collect your card details."} action={!data.has_stripe_customer?"Choose a plan":undefined} click={()=>setTab("plans")}/>}
    </section>}
...
function PlanCard({plan,interval,active,stripeReady,canManage,busy,choose}:{plan:Plan;interval:"month"|"year";active:boolean;stripeReady:boolean;canManage:boolean;busy:string|null;choose:()=>void}) {
