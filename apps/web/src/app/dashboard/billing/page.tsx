...
type Plan = { id:string; code:string; name:string; description:string|null; monthly_price_minor:number; annual_price_minor:number; annual_discount_months?:number; currency:string; trial_days:number; trial_message_limit:number; badge:string|null; cta_label:string; is_featured:boolean; is_self_serve:boolean; is_public:boolean; features:string[]|null; location_limit:number; template_limit:number; automation_limit:number; automation_step_limit:number; media_template_limit:number; review_destination_limit:number; monthly_customer_limit:number|null; stripe_monthly_price_id:string|null; stripe_annual_price_id:string|null };
...
export default function BillingPage() {
  ...
  return <div className="mx-auto max-w-7xl">
    ...
    {!data.stripe_ready&&<Notice text="Online billing is temporarily unavailable while B Reviews finishes its Stripe setup. Your current plan information is still shown." tone="warning"/>}
    ...
    {tab==="plans"&&<div>
      ...
      {customizing&&<div className="fixed inset-0 z-50 overflow-y-auto bg-ink/60 p-3 sm:p-8" role="dialog" aria-modal="true" aria-label="Customize your B Reviews plan"><div className="mx-auto max-w-5xl"><CustomPlanBuilder quote={customQuote} busy={busy} canManage={canManage} calculate={calculateCustomPlan} activate={activateCustomPlan} close={()=>{quoteSequence.current++;setCustomizing(false);setCustomQuote(null);}}/></div></div>}
    </div>}

    {tab==="payment"&&<section className="mt-6 overflow-hidden rounded-2xl border border-ink/8 bg-white">
      <header className="flex flex-wrap items-center justify-between gap-4 border-b border-ink/8 p-6"><div><h2 className="text-xl font-semibold">Payment methods</h2><p className="mt-1 text-sm text-ink/45">Card details are entered in Stripe&apos;s secure form without leaving B Reviews.</p></div>{canManage&&data.stripe_ready&&<button className="button-primary" disabled={Boolean(busy)} onClick={openPaymentMethod}>{busy==="payment"?"Opening secure form…":primary?"Change payment method":"Add payment method"}</button>}</header>
...
function PlanCard({plan,interval,active,assigned,currentPlan,stripeReady,canManage,busy,trialEligible,choose}:{plan:Plan;interval:"month"|"year";active:boolean;assigned:boolean;currentPlan:Plan|null;stripeReady:boolean;canManage:boolean;busy:string|null;trialEligible:boolean;choose:()=>void}) {
  ...
  return <article className={`relative flex flex-col rounded-2xl bg-white p-6 shadow-sm ${assigned?"border-2 border-forest ring-4 ring-forest/10":plan.is_featured?"border border-lime ring-2 ring-lime/25":"border border-ink/8"}`}>
    ...
    <p className="eyebrow">{active?"Active subscription":assigned?"Assigned package":plan.code}</p><h2 className="mt-2 text-2xl font-semibold">{plan.name}</h2>
      <p className="mt-2 min-h-12 text-sm leading-6 text-ink/50">{plan.description}</p><p className="mt-6 text-4xl font-semibold">{money(amount,plan.currency)}<span className="text-sm font-normal text-ink/40"> / {interval==="year"?"year":"month"}</span></p>{interval==="year"&&<p className="mt-2 inline-flex w-fit rounded-full bg-red-50 px-2.5 py-1 text-[11px] font-bold text-red-700">Save {plan.annual_discount_months??2} months</p>}
