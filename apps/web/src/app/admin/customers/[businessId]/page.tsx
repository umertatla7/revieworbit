...
export default function AdminCustomerDetailPage() {
  ...
  return <div className="mx-auto max-w-[1380px]">
    ...
    <div className="mt-6 grid gap-5 xl:grid-cols-[1.15fr_.85fr]">
      <section className="rounded-xl border border-ink/8 bg-white p-5"><p className="eyebrow">Account profile</p><h2 className="mt-1 text-lg font-semibold">Business and owner</h2><dl className="mt-5 grid gap-4 text-sm sm:grid-cols-2"><Info label="Owner" value={business.owners[0]?.name ?? "Not assigned"}/><Info label="Owner email" value={business.owners[0]?.email ?? "—"}/><Info label="Business email" value={business.primary_email || "—"}/><Info label="Business phone" value={business.phone || "—"}/><Info label="Industry" value={business.industry?.replaceAll("_", " ") || "—"}/><Info label="Time zone" value={business.default_timezone}/><Info label="Operation mode" value={business.operation_mode}/><Info label="POS connections" value={String(business.pos_integrations_count)}/></dl></section>
