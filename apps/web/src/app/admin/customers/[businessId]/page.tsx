...
type BusinessDetail = {
  ...
  entitlements: { plan_name: string; location_limit: number; locations_used: number; template_limit: number; templates_used: number; media_template_limit: number; media_templates_used: number; review_destination_limit: number; review_destinations_used: number; monthly_customer_limit: number | null; customers_used_this_month: number };
...
function Usage({ label, used, limit }: { label: string; used: number; limit: number | null }) { const percent = Math.min(100, limit ? (used / limit) * 100 : 0); return <div><div className="flex justify-between text-xs"><span>{label}</span><strong>{used} / {limit ?? "Custom"}</strong></div><div className="mt-2 h-2 overflow-hidden rounded-full bg-paper"><div className="h-full rounded-full bg-forest" style={{ width: `${percent}%` }}/></div></div>; }
