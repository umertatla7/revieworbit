...
describe("platform customer analytics", () => {
  ...
  it("lets a super admin reset the owner password from the customer record", async () => {
    vi.mocked(api).mockResolvedValueOnce({ message: "The owner password was reset and existing sessions were signed out." } as never);
    render(<AdminCustomerDetailPage />);
    fireEvent.click(await screen.findByRole("button", { name: "Reset owner password" }));
    fireEvent.change(screen.getByLabelText("Temporary password"), { target: { value: "NewPassword2026!" } });
    fireEvent.change(screen.getByLabelText("Confirm temporary password"), { target: { value: "NewPassword2026!" } });
    fireEvent.click(screen.getByRole("button", { name: "Reset password" }));

    await waitFor(() => expect(api).toHaveBeenLastCalledWith(
      "/api/v1/admin/businesses/business-1/owners/owner-1/password",
