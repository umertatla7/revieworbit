...
type Usage = { business_id: string; business_name: string; plan_code: string; messages: number; credits_used: number; included_credits: number };
...
const emptyPlan: Partial<Plan> = {
  ...
  is_featured: false, sort_order: 10, features: [], status: "draft",
  location_limit: 1, template_limit: 5, automation_limit: 1,
  automation_step_limit: 2, media_template_limit: 1,
  review_destination_limit: 1, included_message_credits: 100,
...
export default function AdminBillingPage() {
  ...
  async function save(data: FormData) {
    ...
    const numberKeys = [
      ...
      "media_template_limit", "review_destination_limit", "included_message_credits",
    ];
  ...
  return (
    <div className="mx-auto max-w-7xl">
      ...
      {plans.length === 0 ? (
        <section className="mt-6 rounded-2xl border border-dashed border-ink/15 bg-white p-10 text-center"><h2 className="text-xl font-semibold">No plans yet</h2><p className="mt-2 text-sm text-ink/50">Create Basic, Growth, and Pro packages, then sync each active plan with Stripe.</p></section>
      ) : (
      ...
      <section className="mt-7 overflow-hidden rounded-2xl border border-ink/8 bg-white">
        <header className="border-b border-ink/8 p-5"><h2 className="font-semibold">Customer package usage this month</h2><p className="mt-1 text-xs text-ink/45">Message usage is included in each package and is shown here for capacity monitoring.</p></header>
        <div className="divide-y divide-ink/8">{usage.length === 0 ? <p className="p-5 text-sm text-ink/50">No message usage recorded this month.</p> : usage.map((row) => <div key={row.business_id} className="grid gap-2 px-5 py-4 text-sm sm:grid-cols-[1.5fr_.7fr_.7fr_1fr]"><strong>{row.business_name}</strong><span className="capitalize">{row.plan_code}</span><span>{row.messages} sent</span><span>{row.credits_used}/{row.included_credits} included messages</span></div>)}</div>
      </section>
...
function PlanDialog({ plan, busy, close, save }: { plan: Partial<Plan>; busy: boolean; close: () => void; save: (data: FormData) => void }) {
  return <div className="fixed inset-0 z-50 grid place-items-center bg-ink/60 p-4"><form action={save} className="max-h-[94vh] w-full max-w-4xl overflow-auto rounded-2xl bg-white p-6">
    ...
    <div className="mt-4 rounded-xl bg-paper p-4 text-sm text-ink/65">Customers pay only the subscription price. SMS and image messages consume the included package allowance; no per-message fee is added.</div>
    <div className="mt-6 grid gap-4 sm:grid-cols-2">
      ...
      <Field name="badge" label="Badge (optional)" value={plan.badge ?? ""} required={false} /><Field name="sort_order" label="Display order" value={plan.sort_order} number />
