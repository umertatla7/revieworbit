      ...
      {creating && (
        ...
        >
          <form action={createBusiness} className="mt-6 space-y-7">
            ...
            <FormSection number="03" title="Account owner" description="The person responsible for this Breviews workspace.">
              <div className="grid gap-4 sm:grid-cols-3"><FormField label="Owner full name" required><input name="owner_name" className="field" required autoComplete="name" placeholder="Ava Morgan" /></FormField><FormField label="Owner email" required><input name="owner_email" className="field" type="email" required autoComplete="email" placeholder="ava@harbordental.com" /></FormField><FormField label="Owner mobile" hint="Optional · E.164"><input name="owner_phone" className="field" type="tel" pattern="\+[1-9][0-9]{7,14}" placeholder="+12025550124" /></FormField></div><label className="mt-4 flex items-start gap-3 rounded-xl border border-ink/8 bg-paper p-4 text-xs leading-5"><input className="mt-0.5" type="checkbox" name="send_owner_setup_email" defaultChecked/><span><strong className="block text-ink">Send password setup email</strong><span className="text-ink/45">The owner receives a secure password-reset link. Existing Breviews users are attached without changing their password.</span></span></label>
            </FormSection>
