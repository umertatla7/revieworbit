        ...
        >
          <form action={createBusiness} className="mt-6 space-y-7">
            ...
            <FormSection number="02" title="Business contact" description="Operational contact details for the account and default sender profile.">
              <div className="grid gap-4 sm:grid-cols-2"><FormField label="Business email" required><input name="business_email" className="field" type="email" required placeholder="hello@harbordental.com" /></FormField><FormField label="Business phone" required hint="Use international E.164 format"><input name="business_phone" className="field" type="tel" required pattern="\+[1-9][0-9]{7,14}" placeholder="+12025550123" /></FormField></div>
            </FormSection>
            <FormSection number="03" title="Account owner" description="The person responsible for this ReviewOrbit workspace.">
              <div className="grid gap-4 sm:grid-cols-3"><FormField label="Owner full name" required><input name="owner_name" className="field" required autoComplete="name" placeholder="Ava Morgan" /></FormField><FormField label="Owner email" required><input name="owner_email" className="field" type="email" required autoComplete="email" placeholder="ava@harbordental.com" /></FormField><FormField label="Owner mobile" hint="Optional · E.164"><input name="owner_phone" className="field" type="tel" pattern="\+[1-9][0-9]{7,14}" placeholder="+12025550124" /></FormField></div><label className="mt-4 flex items-start gap-3 rounded-xl border border-ink/8 bg-paper p-4 text-xs leading-5"><input className="mt-0.5" type="checkbox" name="send_owner_setup_email" defaultChecked/><span><strong className="block text-ink">Send password setup email</strong><span className="text-ink/45">The owner receives a secure password-reset link. Existing ReviewOrbit users are attached without changing their password.</span></span></label>
            </FormSection>
            ...
            <FormSection number="05" title="Service defaults" description="Initial delivery and integration preferences; these remain editable during onboarding.">
              <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4"><FormField label="POS setup" required><select name="operation_mode" className="field" defaultValue="square"><option value="square">Connect Square</option><option value="generic">Generic API / POS</option><option value="manual">Manual visits</option></select></FormField><FormField label="Preferred channel" required><select name="preferred_channel" className="field" defaultValue="sms"><option value="sms">SMS</option><option value="whatsapp">WhatsApp</option></select></FormField><FormField label="Quiet hours start" required><input name="quiet_hours_start" className="field" type="time" defaultValue="20:00" required /></FormField><FormField label="Quiet hours end" required><input name="quiet_hours_end" className="field" type="time" defaultValue="09:00" required /></FormField></div><FormField label="Internal setup notes" hint="Optional · visible to platform staff only"><textarea name="account_notes" className="field mt-1 min-h-24 resize-y" maxLength={2000} placeholder="Customer requested concierge Square setup. Main contact is available weekday mornings." /></FormField>
            </FormSection>
