...
export function PosSetup({ onChanged }: { onChanged?: () => void }) {
  ...
  async function createToastCode() {
    ...
    try {
      ...
      setMessage("Toast location code created and copied. Open Toast, add Breviews, choose the restaurant location, and paste this code as the Location ID.");
  ...
  return (
    <div className="space-y-6">
      ...
      <section className="overflow-hidden rounded-2xl border border-ink/10 bg-white">
        <div className="grid lg:grid-cols-[1.2fr_.8fr]">
          <div className="p-6 lg:p-7">
            ...
            <p className="mt-5 max-w-2xl text-sm leading-6 text-ink/60">Import Square locations, customer contact profiles, previous appointments, and upcoming appointments. Breviews never treats a scheduled appointment as a completed visit or messaging consent.</p>
      ...
      <section className="overflow-hidden rounded-2xl border border-ink/10 bg-white">
        <div className="grid lg:grid-cols-[1.2fr_.8fr]">
          <div className="p-6 lg:p-7"><div className="flex items-center gap-3"><span className="grid size-11 place-items-center rounded-xl bg-[#f05a28] text-lg font-bold text-white">T</span><div><div className="flex items-center gap-2"><h2 className="text-xl font-semibold">Toast POS</h2><span className="pill capitalize">{toastProvider?.availability ?? "Loading"}</span></div><p className="mt-1 text-sm text-ink/50">Partner connection · one code per restaurant location</p></div></div>
            <p className="mt-5 max-w-2xl text-sm leading-6 text-ink/60">Toast connections are location-based. Breviews imports completed paid checks as visits. Guest contact information is used only to match the customer and never counts as SMS consent.</p>
            <div className="mt-6 flex flex-col gap-3 sm:flex-row sm:items-end"><label className="label flex-1">Breviews location<select className="field" value={toastLocationId} onChange={(event) => setToastLocationId(event.target.value)}><option value="">Select a location</option>{locations.map((location) => <option key={location.id} value={location.id}>{location.name}</option>)}</select></label><button className="button-primary sm:mb-px" disabled={busy !== null || !toast?.ready || !toastLocationId} onClick={createToastCode}>{busy === "toast" ? "Creating code…" : "Create Toast location code"}</button></div>
      ...
      {credentials && <section className="rounded-2xl bg-ink p-5 text-white"><p className="font-semibold text-mint">Copy credentials now</p><p className="mt-1 text-xs text-white/55">Only hashes and encrypted secrets are stored by Breviews.</p><Credential label="API key" value={credentials.api_key}/><Credential label="Webhook key prefix" value={credentials.key_prefix}/><Credential label="HMAC secret" value={credentials.hmac_secret}/></section>}
    </div>
  );
}
