...
export function CustomerOnboarding() {
  ...
  return <main className="flex min-h-screen flex-col bg-paper pt-8">
    <div className="mx-auto w-full max-w-7xl flex-1 px-6 lg:px-10"><div className="flex items-center justify-between"><Brand compact/><button className="text-sm font-semibold text-forest underline" onClick={() => router.push("/dashboard")}>Finish later</button></div>
      <div className="mt-10 grid gap-8 lg:grid-cols-[300px_1fr]">
        <aside className="h-fit rounded-[2rem] bg-forest p-6 text-white lg:sticky lg:top-8"><p className="text-xs font-bold uppercase tracking-[0.2em] text-[#ffb0b6]">Customer onboarding</p><h1 className="mt-3 text-3xl font-semibold">Launch your workspace</h1><p className="mt-3 text-sm leading-6 text-white/60">Complete the essentials yourself or let B Reviews support configure them with audited access.</p><div className="mt-7 h-2 overflow-hidden rounded-full bg-white/15"><div className="h-full rounded-full bg-mint" style={{ width: `${percent}%` }}/></div><p className="mt-2 text-sm text-white/65">{data?.completed_count ?? 0} of {data?.total_count ?? 7} essentials complete</p><div className="mt-7 space-y-3">{Object.entries(data?.checks ?? {}).map(([key, complete]) => <div key={key} className="flex items-center gap-3 text-sm"><span className={`grid size-6 place-items-center rounded-full ${complete ? "bg-mint text-white" : "bg-white/10 text-white/40"}`}>{complete ? "✓" : "·"}</span><span className={complete ? "text-white" : "text-white/55"}>{checkLabels[key]}</span></div>)}</div></aside>
        <div className="space-y-7"><div><p className="eyebrow">Welcome{data?.business.name ? `, ${data.business.name}` : ""}</p><h2 className="page-title">Set up your customer workspace</h2><p className="page-intro">These settings control tenant identity, review destinations, consent-aware messaging, and how completed visits reach B Reviews.</p></div>{message && <p role="status" className="rounded-2xl border border-forest/15 bg-white px-5 py-4 text-sm text-forest">{message}</p>}
          ...
          <section className="rounded-[2rem] bg-ink p-7 text-white"><p className="text-xs font-bold uppercase tracking-[0.2em] text-[#ffb0b6]">Final check</p><h2 className="mt-3 text-2xl font-semibold">Ready to open the dashboard?</h2><p className="mt-2 text-sm leading-6 text-white/60">You can continue refining locations, templates, customers, and POS settings after onboarding.</p><button className="mt-5 rounded-xl bg-mint px-5 py-3 text-sm font-semibold text-white disabled:opacity-40" disabled={busy || !data || data.completed_count < data.total_count} onClick={complete}>Complete onboarding</button></section>
        </div>
      </div>
    </div><SiteFooter />
  </main>;
