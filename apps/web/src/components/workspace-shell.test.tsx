  ...
  it("renders grouped navigation, status badges, and account controls", () => {
    render(
      <WorkspaceShell
        groups={groups}
        mode="customer"
        userName="AL Barber Shop Owner"
        userDetail="owner@example.com"
        workspaceName="AL Barber Shop"
        onLogout={vi.fn()}
      >
        <h1>Messages</h1>
      </WorkspaceShell>,
    ...
    expect(
      screen.getByRole("link", { name: /Messages\s*Soon/ }),
    ...
    expect(
      screen.getByRole("link", { name: /Analytics\s*Soon/ }),
    ).toBeInTheDocument();
  });
});
