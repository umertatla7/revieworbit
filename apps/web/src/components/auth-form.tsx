...
type SignupPlan = { id:string; name:string; description:string|null; monthly_price_minor:number; annual_price_minor:number; currency:string; trial_days:number; trial_message_limit:number; badge:string|null; cta_label:string; is_self_serve:boolean; location_limit:number; monthly_customer_limit:number|null };
...
function SwitchLink({mode}:{mode:"login"|"register"}){return <p className="mt-6 text-center text-sm text-ink/60">{mode==="login"?"New to Breviews?":"Already have an account?"} <Link className="font-semibold text-forest underline" href={mode==="login"?"/register":"/login"}>{mode==="login"?"Create an account":"Sign in"}</Link></p>}
