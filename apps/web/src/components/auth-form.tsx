...
type SignupPlan = { id:string; name:string; description:string|null; monthly_price_minor:number; annual_price_minor:number; currency:string; trial_days:number; badge:string|null; cta_label:string; is_self_serve:boolean; location_limit:number; monthly_customer_limit:number|null };
...
export function AuthForm({ mode }: { mode: "login" | "register" }) {
  ...
  return <AuthShell wide eyebrow="Start your workspace" title={step===1?"Tell us about your business":step===2?"Choose your plan":"Secure payment"}>
    ...
    {step===2&&<form className="mt-7" onSubmit={handleSubmit(submit)}><div className="mb-5 flex items-center justify-between gap-4"><p className="text-sm text-ink/50">Every self-serve trial requires a card. You will not be charged until the trial ends.</p><div className="flex rounded-xl bg-paper p-1"><button type="button" className={`rounded-lg px-4 py-2 text-sm font-semibold ${interval==="month"?"bg-forest text-white":""}`} onClick={()=>setInterval("month")}>Monthly</button><button type="button" className={`rounded-lg px-4 py-2 text-sm font-semibold ${interval==="year"?"bg-forest text-white":""}`} onClick={()=>setInterval("year")}>Annual</button></div></div><div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">{plans.map(plan=>{const selected=selectedPlan===plan.id;const amount=interval==="year"?plan.annual_price_minor:plan.monthly_price_minor;return <article key={plan.id} className={`relative rounded-2xl border p-5 text-left ${selected?"border-2 border-forest ring-4 ring-forest/10":"border-ink/10"}`}><span className="eyebrow">{selected?"Selected":plan.badge??"Plan"}</span><strong className="mt-2 block text-xl">{plan.name}</strong><span className="mt-4 block text-3xl font-semibold">{plan.is_self_serve?money(amount,plan.currency):"Custom"}<small className="text-xs font-normal text-ink/45">{plan.is_self_serve?` / ${interval}`:""}</small></span><p className="mt-3 min-h-16 text-xs leading-5 text-ink/55">{plan.description}</p><span className="mt-3 block text-xs text-forest">{plan.is_self_serve?`${plan.trial_days}-day trial · card required`:"Tailored onboarding and pricing"}</span><span className="mt-4 block text-xs leading-5 text-ink/55">{plan.monthly_customer_limit?`${plan.monthly_customer_limit.toLocaleString()} customers / month` : "Custom customer volume"} · {plan.location_limit}+ location{plan.location_limit===1?"":"s"}</span>{plan.is_self_serve?<button type="button" onClick={()=>setValue("plan_id",plan.id)} className="mt-5 w-full rounded-xl bg-forest px-3 py-2 text-sm font-semibold text-white">{plan.cta_label}</button>:<a href="mailto:hello@revieworbit.tech?subject=ReviewOrbit%20Enterprise" className="mt-5 block rounded-xl border border-forest px-3 py-2 text-center text-sm font-semibold text-forest">{plan.cta_label}</a>}</article>})}</div>{serverError&&<ErrorNotice text={serverError}/>}<div className="mt-6 flex gap-3"><button type="button" className="rounded-xl border border-ink/10 px-5 py-3 font-semibold" onClick={()=>setStep(1)}>Back</button><button className="button-primary flex-1" disabled={isSubmitting||!selectedPlan}>{isSubmitting?"Creating secure checkout…":"Continue to secure payment"}</button></div></form>}
