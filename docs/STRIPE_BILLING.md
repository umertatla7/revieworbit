...
Super administrators configure test or live credentials at `/admin/stripe`. Secret and webhook keys are encrypted at rest and are never returned by the API. Verify the account, configure plans in `/admin/billing`, then synchronize one active self-service plan or all active self-service plans. The admin page explicitly distinguishes the real ReviewOrbit database record from its Stripe publication state: **Not synchronized**, **Partial**, or **Published**. A self-service plan is represented by one Stripe Product and immutable monthly and annual Prices. A price change creates a replacement Price and deactivates the previous Price. Sales-assisted custom plans are not synchronized. Stripe API validation errors are returned as actionable form errors instead of generic server errors.
...
The initial catalogue is Launch ($49/month, 100 customers), Momentum ($99/month, 300 customers), Expansion ($139/month, 400 customers shared across two locations), and configurable Custom. Public Custom shows no misleading fixed allowance. A tenant owner enters customer volume, locations, messaging volume, MMS mix, and support needs in Plan & billing; ReviewOrbit calculates the price, creates a tenant-private package, publishes its Stripe prices, and continues to embedded Checkout. A private package can only be viewed or purchased by its owning tenant. Administrators can use the same model for assisted sales and may change prices, limits, display order, badges, trial length, trial message allowance, CTA labels, and availability. Customer volume counts unique customers whose review journey begins during the calendar month; follow-up messages for an already-counted customer are included.

The first paid subscription for a business may use a seven-day, card-required trial with ten ReviewOrbit-branded test messages. `trial_used_at` permanently records consumption of that first trial. Upgrades, downgrades, reactivations, and later custom packages never receive another trial. Live manual and automated customer sends remain locked until activation, and the API blocks the eleventh test. This cap limits provider exposure while allowing number rental and A2P registration for the customer's live sender to be delayed until conversion.
