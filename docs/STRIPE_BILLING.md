...
Annual pricing is derived from the monthly price and the plan's configurable `annual_discount_months` value. The customer-facing default is two free months, so a $49 monthly package publishes an annual price of $490. Administrators configure the free-month discount rather than entering a separate annual total, preventing the displayed amount and Stripe price from drifting apart.
