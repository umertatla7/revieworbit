...
Plan entitlements are resolved server-side from the business `plan_code`; the UI is not trusted to enforce allowances. Basic includes one location and Google, Growth includes five locations plus Google/Trustpilot/Facebook, and Pro includes 25 locations plus Google/Trustpilot/Facebook/Yelp/custom destinations. `location_review_destinations` is tenant-owned and unique by location/provider. The legacy location Google URL remains synchronized while messaging transitions to the provider-neutral destination relation.
