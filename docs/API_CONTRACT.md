...
- Super administrators configure encrypted Stripe credentials at `/api/v1/admin/stripe`, create and update plans under `/api/v1/admin/plans`, calculate internal custom quotes with `POST /api/v1/admin/plans/quote`, and explicitly synchronize immutable Stripe prices with `POST /api/v1/admin/plans/{plan}/stripe-sync`.
