...
- `GET /api/v1/billing` returns the authenticated tenant's active public plan catalogue, any tenant-owned private custom plan, synchronized subscription state, masked payment methods, and invoice history. Owners can price requirements with `POST /api/v1/billing/custom-quote` and create or update their Stripe-backed private package with `POST /api/v1/billing/custom-plan`; internal provider-cost and margin inputs are never returned. A tenant owner—or a super administrator in an active, audited support session—may start embedded checkout and payment-method setup through `POST /api/v1/billing/checkout` and `POST /api/v1/billing/payment-method`. Private custom plans are viewable and purchasable only by their owning business.
- Super administrators configure encrypted Stripe credentials at `/api/v1/admin/stripe`, create and update plans under `/api/v1/admin/plans`, calculate internal custom quotes with `POST /api/v1/admin/plans/quote`, and create, synchronize, and assign a tenant-private quote with `POST /api/v1/admin/plans/custom-assign`. Assigning to an active Stripe subscriber changes the subscription with prorations and no new trial. Immutable standard-plan prices are explicitly synchronized with `POST /api/v1/admin/plans/{plan}/stripe-sync`.
